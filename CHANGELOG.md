...
- `--format json` on the `audit` subcommand: the report renderer can now emit
  one JSON document with a header object and a findings array, which makes the
  auditor safe to pipe into alerting pipelines.
